...
# Definindo o background azul desde o início para a aplicação e sidebar
st.markdown(
    """
    <style>
    .stApp {
...
# Função para verificar se as sequências estão alinhadas
...
# Função para criar diretório único para modelos
...
# Função para realinhar sequências utilizando MAFFT
...
def format_and_sum_probabilities(associated_rankings: list) -> tuple:
    ...
    Os valores extraídos (em percentuais) são convertidos para decimais e, em seguida, normalizados
    dividindo-se pela soma total (garantindo que a soma dos top 3 seja uma fração da distribuição).
...
def plot_umap_3d_combined(X_original: np.ndarray, X_synthetic: np.ndarray, 
    ...
    Each sample is colored based on its associated variable. Original and synthetic samples
    share the same base color for a given associated variable, but synthetic samples are shown in a lighter shade.
    The legend displays the total number of samples for each variable in each group.
    
    Parameters:
    - X_original (np.ndarray): Original embeddings.
    - X_synthetic (np.ndarray): Synthetic embeddings.
    - var_assoc_original (list): Associated variables for original samples.
    - var_assoc_synthetic (list): Associated variables for synthetic samples.
    - output_path (str, optional): If provided, saves the plot as an HTML file at this path.
    ...
    # Create DataFrame with UMAP coordinates and group info
    df = pd.DataFrame({
        'UMAP1': X_umap[:, 0],
        'UMAP2': X_umap[:, 1],
        'UMAP3': X_umap[:, 2],
    ...
    # Define a base color mapping for each associated variable using Plotly palette
    ...
    # Assign a color to each row: original uses base color; synthetic uses a lighter tone
    def assign_color(row):
        base = base_colors.get(row['Associated Variable'], '#808080')
        if row['Group'] == 'Original':
            return base
        else:
            return lighten_color(base, 0.5)
    
    df['Color'] = df.apply(assign_color, axis=1)
    
    # Create a 3D scatter plot grouped by 'Associated Variable' and 'Group'
    fig = go.Figure()
    for var in unique_vars:
        # Original samples for the variable
        ...
        # Synthetic samples for the variable
    ...
    fig.update_layout(
        title=f"3D UMAP (Original: {n_orig} samples, Synthetic: {n_syn} samples)",
        scene=dict(
...
def plot_umap_2d_combined(X_original: np.ndarray, X_synthetic: np.ndarray, 
                            ...
                            output_path: str = None) -> go.Figure:
    """
    Generates a 2D UMAP plot combining original and synthetic samples (after initial oversampling)
    prior to cross-validation.
    
    Each point is colored according to its associated variable and the symbol indicates the group
    (Original or Synthetic). Original and synthetic samples for a given associated variable share the same
    base color, but synthetic samples are shown in a lighter tone.
    
    The title shows the number of samples in each group.
    ...
    Returns:
    - go.Figure: Interactive 2D Plotly figure.
    """
    # Combine the data
    X_combined = np.vstack([X_original, X_synthetic])
    n_orig = X_original.shape[0]
    ...
    # Apply UMAP for 2D with adjusted parameters
    umap_reducer = umap.UMAP(n_components=2, random_state=42, n_neighbors=30, min_dist=0.05)
    X_umap = umap_reducer.fit_transform(X_combined)
    
    # Create a DataFrame with UMAP coordinates and group info
    df = pd.DataFrame({
        'UMAP1': X_umap[:, 0],
        'UMAP2': X_umap[:, 1],
        'Associated Variable': np.concatenate([var_assoc_original, var_assoc_synthetic]),
    ...
    # Define base color mapping for each associated variable
    unique_vars = sorted(set(var_assoc_original).union(set(var_assoc_synthetic)))
    base_colors = {}
    palette = px.colors.qualitative.Plotly
    for i, var in enumerate(unique_vars):
    ...
    # Create the 2D scatter plot grouped by variable and group
    fig = go.Figure()
    for var in unique_vars:
        df_orig = df[(df['Group'] == 'Original') & (df['Associated Variable'] == var)]
        if not df_orig.empty:
    ...
    fig.update_layout(
        title=f"UMAP 2D (Original: {n_orig} samples, Synthetic: {n_syn} samples)",
        xaxis=dict(title="UMAP Dimension 1"),
        yaxis=dict(title="UMAP Dimension 2"),
        template="plotly_dark"
    )
...
def visualize_latent_space_with_similarity(
    ...
    """
    Visualizes the latent space using 3D UMAP and displays cosine similarity between original and synthetic samples,
    considering only samples from the same class (associated variable). Each associated variable is assigned a distinct
    color, and the legend shows the total number of original and synthetic samples for that variable.
    
    Users can click on any legend item to toggle (hide/show) that trace and assess the overlap between original and synthetic samples.
    ...
    - protein_ids_original (list): Protein IDs for original samples.
    - protein_ids_synthetic (list): Protein IDs for synthetic samples.
    - var_assoc_original (list): Associated variables for original data.
    - var_assoc_synthetic (list): Associated variables for synthetic data.
    ...
    # For each synthetic sample, calculate the cosine similarity with original samples of the same class
    similarities_list = []
    ...
    # Create the interactive 3D figure with Plotly
    fig = go.Figure()
    
    # For each class, add original and synthetic traces with updated legend names
    ...
    # Save the plot as an HTML file if output_dir is provided
    if output_dir:
...
class Support:
    ...
    def fit(
        ...
        X_original = X
        X_synthetic = X_smote[len(X):]  # Assumindo que os dados sintéticos vêm após os originais
        var_assoc_original = var_assoc  # Lista original das variáveis associadas
        var_assoc_synthetic = synthetic_var_assoc  # Lista das variáveis associadas para os sintéticos
        ...
        fig_umap_2d = plot_umap_2d_combined(
            X_original, X_synthetic, var_assoc_original, var_assoc_synthetic,
            output_path=os.path.join(model_dir, "umap_2d_pre_cv.html")
            )
...
#        scaler_path = os.path.join(scaler_dir, 'scaler_associated.pkl')
    ...
    # and the rest are synthetic.
        logging.info("Visualizing latent space afRFter oversampling inside test_best_RF...")
        fig_latent = visualize_latent_space_with_similarity(
            X_original = X_scaled, 
            X_synthetic = X_resampled[len(y):],
            y_original = y, 
    ...
    # Calculate per-class F1 scores:
        ...
        logging.info(f"Global F1 Score (weighted): {f1_global}")
...
# Gera as barras com as cores correspondentes
        for i, cls in enumerate(unique_classes):
            ...
            ax_bar.bar(cls, score_val, color=viridis_colors[i])
            ax_bar.text(i, score_val + 0.02, f"{score_val:.2f}", ha='center', fontsize=12)
    
        ax_bar.set_title('F1 Score per Class', fontsize=16)
        ax_bar.set_xlabel('Associated Variable (Class)', fontsize=14)
        ax_bar.set_ylabel('F1 Score', fontsize=14)
        ax_bar.set_ylim(0, 1)
        plt.tight_layout()

# Define os caminhos para salvar os arquivos em PNG e SVG no diretório model_dir
...
#############
    ...
    # Cria DataFrames para training e test
        ...
        sns.violinplot(x='Associated Variable', y='Probability', hue='Dataset', data=df_violin,
                       split=True, palette='viridis', ax=ax_violin)
...
def plot_predict_umap(
    ...
    """
    Plots a 3D UMAP projection for prediction embeddings only and saves it as an HTML file.
    
    The hover text for each point displays the Protein ID and the Top Category (i.e., the top-sum
    associated variable as shown in the normalized table).
    
    Parameters:
    - predict_embeddings (np.ndarray): Prediction embeddings.
    - predict_labels (list): Top-sum category labels for each prediction.
    - predict_protein_ids (list): Protein IDs for prediction samples.
    - output_dir (str): Directory to save the UMAP plot (optional).
    
    Returns:
    - go.Figure: Interactive Plotly figure for predictions.
    """
    # Dimensionality reduction using UMAP with chosen parameters
    ...
    # Create color mapping for prediction labels
    ...
    # Create the interactive 3D UMAP plot for predictions
    ...
    fig_predict.add_trace(go.Scatter3d(
        ...
        text=[f"Protein ID: {pid}<br>Top Category: {label}" 
              for pid, label in zip(predict_protein_ids, predict_labels)],
        hoverinfo='text',
        name='Predictions'
    ))
    ...
    # Save the figure as an HTML file if output_dir is provided
    if output_dir:
...
def plot_prediction_confidence_bar(df_results: pd.DataFrame) -> None:
    """
    Cria um gráfico de barras horizontal interativo para visualizar a confiança da predição.
    O eixo x varia de 0 a 1, e cada barra representa o valor de 'Prediction Confidence'.
    Cada barra é colorida de acordo com o nível de confiança:
      - Baixa Confiança (0 - 0.3)
      - Média Confiança (0.3 - 0.5)
      - Alta Confiança (0.5 - 1)
      
    A legenda única explica os intervalos de confiança.
    
    Parâmetros:
    - df_results (pd.DataFrame): DataFrame contendo as colunas "Query Name" e "Prediction Confidence ( range: 0 - 1 )"
    """
    # Converter a coluna de confiança para float
    df_results['Prediction Confidence'] = df_results['Prediction Confidence ( range: 0 - 1 )'].astype(float)
    
    # Função para determinar o nível de confiança com base no valor
    def get_confidence_level(confidence):
        if confidence < 0.3:
            return "Baixa Confiança (0 - 0.3)"
        elif confidence < 0.5:
            return "Média Confiança (0.3 - 0.5)"
        else:
            return "Alta Confiança (0.5 - 1)"
    
    # Aplicar a função para criar uma nova coluna de nível de confiança
    df_results['confidence_level'] = df_results['Prediction Confidence'].apply(get_confidence_level)
    
    # Definir mapeamento de cores para cada nível de confiança, escolhidas para harmonizar com o fundo azul
    color_map = {
        "Baixa Confiança (0 - 0.3)": "#F4D03F",  # Amarelo dourado
        "Média Confiança (0.3 - 0.5)": "#E67E22",  # Laranja
        "Alta Confiança (0.5 - 1)": "#3498DB"       # Azul claro
    }
    
    # Criar o gráfico de barras horizontal usando Plotly Express
    fig_conf = px.bar(
        df_results,
    ...
    # Atualizar o layout do gráfico para deixar títulos, eixos e legendas com texto branco
    fig_conf.update_layout(
        ...
        legend_title_text="Intervalos de Confiança",
        legend=dict(
            font=dict(color='white')
        )
    )
    ...
    # Botão de download para a figura
    st.download_button(
...
def main(args: argparse.Namespace) -> None:
    ...
    protein_embedding_train.generate_embeddings(
        ...
        window=5,   # Ajustar se necessário (ou via args)
        workers=48, # Ajustar se necessário
        epochs=2500 # Ajustar se necessário
    )
    ...
    logging.debug("O valor is %s", predict_labels)
    ...
    # Mantemos a função, mas sem subset de background_gradient
    ...
    st.markdown(
        """
        <style>
        /* Forçar cor branca em todos os labels, inclusive o do checkbox "Use Default Training Data" */
    ...
    # NOVA SEÇÃO: Exibir gráfico de barras horizontal para Prediction Confidence
    ...
    # Salvar os Dual UMAP plots na pasta de saída para inclusão no ZIP
    ...
    # Salvar os Dual tsNE plots na pasta de saída para inclusão no ZIP    
...
st.markdown(
    ...
    <div style="text-align: center; font-family: 'Arial', sans-serif; padding: 30px; background: linear-gradient(to bottom, #f9f9f9, #ffffff); border-radius: 15px; border: 2px solid #dddddd; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); position: relative;">
        ...
        <p style="color: #2c3e50; font-size: 18px; line-height: 1.8; text-align: justify;">
            ...
            (Kurt et al., 2017; Gilmore et al., 2012). Dr. Pedro Leão team (<a href="https://leaolab.wixsite.com/leaolab" target="_blank" style="color: #3498db; text-decoration: none;">visit here</a>)
        </p>
...
# -- CSS para forçar texto branco em todos os labels
# 2. CSS injetado ANTES de criar elementos no sidebar
# CSS injetado ANTES de criar o checkbox
# -- CSS para forçar texto branco em todos os labels, spans, etc. no sidebar
# Força que os textos dos rótulos (labels) dos elementos da sidebar fiquem em branco
# Força que os textos dos rótulos (labels) dos elementos da sidebar fiquem em branco
# Força que os textos dos rótulos (labels) dos elementos da sidebar fiquem em branco
st.sidebar.markdown(
    """
...
# =====================================
# Sessão de Input (barra lateral)
# =====================================
st.sidebar.header("Input Parameters")

st.sidebar.header("Use Default Training Data")
# Parâmetro 1: Escolha se deseja usar dados de treinamento padrão
...
if not use_default_train:
    # Se o usuário desmarcar, pedimos o upload dos arquivos
...
# Atualiza a contagem e obtém o país do visitante
visit_count = update_visit_count()
visitor_country = get_visitor_country()

# Exibe a contagem de visitas e o país no rodapé
st.markdown(
    f"""
