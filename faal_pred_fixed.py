...
class Support:
    ...
    def fit(
        ...
        X_original = X
        X_synthetic = X_smote[len(X):]  # Assumindo que os dados sintÃ©ticos vÃªm apÃ³s os originais
        var_assoc_original = var_assoc  # Lista original das variÃ¡veis associadas
        var_assoc_synthetic = synthetic_var_assoc  # Lista das variÃ¡veis associadas para os sintÃ©ticos
...
############################################# Save the model in the final test
    def test_best_RF(
        ...
        - protein_ids (list): Lista de IDs das proteÃ­nas correspondentes a X.
        - var_assoc (list): Lista de variÃ¡veis associadas correspondentes a X.
        ...
        """
    # Define o caminho do scaler usando scaler_dir
    ...
    # Aplicar oversampling em todo o conjunto antes da divisÃ£o
    ...
    # VisualizaÃ§Ã£o do espaÃ§o latente (assume que as primeiras len(y) amostras sÃ£o originais)
        logging.info("Visualizing latent space after oversampling inside test_best_RF...")
    ...
    # Treinamento do RandomForest com os melhores parÃ¢metros encontrados
    ...
    # CalibraÃ§Ã£o das probabilidades usando Isotonic Regression
    ...
    # CriaÃ§Ã£o do grÃ¡fico de barras para os F1 Scores por classe
        fig_bar, ax_bar = plt.subplots(figsize=(10, 6))
        unique_classes_list = list(f1_class_dict.keys())
        n_classes = len(unique_classes_list)
        viridis_colors = plt.cm.viridis(np.linspace(0, 1, n_classes))

        for i, cls in enumerate(unique_classes_list):
        ...
        png_path = os.path.join(scaler_dir, "f1_per_class.png")
        svg_path = os.path.join(scaler_dir, "f1_per_class.svg")

        fig_bar.savefig(png_path, dpi=300, bbox_inches='tight')
        fig_bar.savefig(svg_path, format='svg', bbox_inches='tight')
        plt.close(fig_bar)

    # --- GeraÃ§Ã£o de Violin Plots para as probabilidades previstas da classe verdadeira ---
        ...
        plt.tight_layout()
        violin_png_path = os.path.join(scaler_dir, "true_probability_violin.png")
        violin_svg_path = os.path.join(scaler_dir, "true_probability_violin.svg")
...
def main(args: argparse.Namespace) -> None:
    ...
    if os.path.exists(calibrated_model_associated_full_path):
        ...
        best_score_associated, best_f1_associated, best_pr_auc_associated, best_params_associated, best_model_associated, X_test_associated, y_test_associated = support_model_associated.test_best_RF(
            X_associated_scaled, 
            y_associated,
            protein_ids_associated,
            var_assoc_associated,
...
# -- CSS para forÃ§ar texto branco em todos os labels
...
# Atualiza a contagem e obtÃ©m o paÃ­s do visitante
visit_count = update_visit_count()
visitor_country = get_visitor_country()

# Exibe a contagem de visitas e o paÃ­s no rodapÃ©
st.markdown(
    f"""
