...
def standardize_lineage_format(lineage_string):
    """
    Normalizes the lineage string ensuring that each ';' is followed by a single space,
    extra spaces are removed, and the string ends with ';'.
    """
...
def get_corrected_lineage_from_species(species_name):
    """
    From the species name (expected to be in the column 'Species'),
    use ete3 to:
      1. Translate the name to taxid.
...
def get_corrected_lineage_from_organism(organism_name):
    """
    Similar to get_corrected_lineage_from_species but uses the organism_name
    coming from the 'Organism Name' column in Table 2.
    """
    return get_corrected_lineage_from_species(organism_name)

def update_lineage_for_eukaryotes_table1(df):
    """
    Updates the 'Lineage' column for rows corresponding to Eukaryota in Table 1,
    using the 'Species' column to get the corrected lineage via ete3.
    """
    if 'Species' not in df.columns:
        raise KeyError("Column 'Species' not found in Table 1 DataFrame.")
...
def update_lineage_for_eukaryotes_table2(df):
    """
    Updates the 'Lineage' column for rows corresponding to Eukaryota in Table 2,
    using the 'Organism Name' column to obtain the corrected lineage via ete3.
    """
    if 'Organism Name' not in df.columns:
        raise KeyError("Column 'Organism Name' not found in Table 2 DataFrame.")
...
def generate_filtered_table_and_graphs(table1_path, table2_path, domain_name, taxonomic_level, top_n, dpi, sub_taxonomic_level=None):
    ...
    # Update lineage for Eukaryota in Table 1 if selected
    ...
    # If Eukaryota is selected, update the lineage using the "Organism Name" column
    if "Eukaryota" in domain_name:
    ...
    # For each taxonomic group, if the count of genomes from Table 1 (Genome_Count)
    # is less than the total genome count from Table 2, recalc the union of genome IDs from both tables.
    updated_total_ids = {}
