...
Let \(A_i(t)\) be the residue of sequence \(i\) at aligned position \(t\), and `-` denote a gap. Identity between sequences \(i\) and \(j\) is:
\[
\mathrm{Id}(i,j) \;=\;
...
\]
For dendrograms from W2V, we use distances such as \(1-\cos(\theta_{ij})\).
