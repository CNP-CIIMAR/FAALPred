...
class Support:
    ...
    def _oversample_single_sample_classes(
        ...
        """
        Oversamples classes using RandomOverSampler and SMOTE.
        Always creates synthetic IDs and var_assoc if not provided.
        """
        ...
        for cls in final_counts:
            ...
            for i in cls_original_indices:
                if i < len(protein_ids_final) and i < len(var_assoc_final):
                    protein_ids_original_only.append(protein_ids_final[i])
                    var_assoc_original_only.append(var_assoc_final[i])
                else:
                    # Fallback sintético
                    protein_ids_original_only.append(f"synthetic_fallback_id_{i}")
                    var_assoc_original_only.append(f"synthetic_fallback_var_{i}")
            ...
            if n_duplicates > 0:
                ...
                for di in cls_duplicate_indices:
                    if di < len(protein_ids_final) and di < len(var_assoc_final):
                        protein_ids_duplicates_all.append(protein_ids_final[di])
                        var_assoc_duplicates_all.append(var_assoc_final[di])
                    else:
                        protein_ids_duplicates_all.append(f"synthetic_fallback_id_{di}")
                        var_assoc_duplicates_all.append(f"synthetic_fallback_var_{di}")
        ...
        def generate_synthetic_samples(X_dup, y_dup, protein_dup, var_dup, seed=42):
            if len(X_dup) == 0:
            ...
            X_dup_shuffled = X_dup[indices]
            y_dup_shuffled = y_dup[indices]

            # Garante fallback sintético se índices ultrapassarem
            def safe_get(lst, i, prefix):
                return lst[i] if i < len(lst) else f"synthetic_{prefix}_{i}"

            protein_dup_shuffled = [safe_get(protein_dup, i, "prot") for i in indices]
            var_dup_shuffled = [safe_get(var_dup, i, "var") for i in indices]

            if len(X_dup_shuffled) > 1:
                k_smote = min(5, len(X_dup_shuffled)-1)
                smote = SMOTE(random_state=seed, k_neighbors=k_smote)
                X_sm, y_sm = smote.fit_resample(X_dup_shuffled, y_dup_shuffled)

                synthetic_prot_sm = []
                synthetic_var_sm = []
                for i in range(len(X_sm)):
                    if i < len(X_dup_shuffled):
                        synthetic_prot_sm.append(protein_dup_shuffled[i])
                        synthetic_var_sm.append(var_dup_shuffled[i])
                    else:
                        synthetic_prot_sm.append(f"synthetic_smote_id_{i}")
                        synthetic_var_sm.append(f"synthetic_smote_var_{i}")
            else:
                X_sm, y_sm = X_dup_shuffled, y_dup_shuffled
                synthetic_prot_sm = protein_dup_shuffled[:]
                synthetic_var_sm = var_dup_shuffled[:]

            return X_sm, y_sm, synthetic_prot_sm, synthetic_var_sm

        X_synth, y_synth, prot_synth_all, var_synth_all = generate_synthetic_samples(
            X_duplicates_all, 
            y_duplicates_all, 
            protein_ids_duplicates_all, 
            var_assoc_duplicates_all, 
        ...
        if X_add:
            ...
            X_done = np.vstack([X_original_only, X_add])
            y_done = np.hstack([y_original_only, y_add])
            protein_ids_done = protein_ids_original_only + prot_add
            var_assoc_done = var_assoc_original_only + var_add
        else:
            X_done, y_done = X_original_only, y_original_only
        ...
        with open("oversampling_counts.txt", "a") as f:
            f.write(f"Class distribution after ensuring min, {final_target}, and SMOTE:\n")
    ...
    def fit(
        ...
        # Cria ID e var sintéticos caso não existam
...
def visualize_latent_space_with_similarity(
    ...
    Includes a fallback if closest_indices exceed range.
    """
    ...
    for idx in closest_indices:
        if idx < len(protein_ids_original) and idx < len(var_assoc_original):
            ...
            # Fallback sintético caso idx seja inválido
