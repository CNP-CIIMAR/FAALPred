...
def plot_roc_curve_global(y_true: np.ndarray, y_pred_proba: np.ndarray, title: str, save_as: str = None, classes: list = None) -> None:
    ...
    else:  # Multiclass classification or multilabel case (aplicando ovo-one versus rest)
...
def get_class_rankings_global(model: RandomForestClassifier, X: np.ndarray) -> list:
    ...
    Adaptado para multilabel: se o problema for multilabel, assume que model.predict_proba retorna uma lista de arrays,
    um para cada rótulo.
    ...
    # Se multilabel, y_pred_proba pode ser uma lista de arrays (um para cada label)
    if isinstance(y_pred_proba, list):
        rankings = []
        n_samples = X.shape[0]
        # Supondo que cada array tenha shape (n_samples, 2) para classificação binária de cada rótulo
        for i in range(n_samples):
            sample_ranking = []
            for j, proba_array in enumerate(y_pred_proba):
                # Considera a probabilidade da classe 1
                prob = proba_array[i, 1]
                sample_ranking.append((model.classes_[j] if hasattr(model, 'classes_') else f"Label_{j}", prob))
            # Ordena por probabilidade decrescente
            sample_ranking.sort(key=lambda x: x[1], reverse=True)
            formatted = [f"{lbl}: {p*100:.2f}%" for lbl, p in sample_ranking]
            rankings.append(formatted)
        return rankings
    else:
        # Caso não seja multilabel, processa como antes
        class_rankings = []
...
class Support:
    ...
    def _oversample_multilabel(self, X: np.ndarray, y: np.ndarray) -> tuple:
        ...
        2) Usa MLSMOTE para gerar amostras sintéticas no contexto multilabel.
        
        Parameters:
        - X (np.ndarray): Features.
        - y (np.ndarray): Labels multilabel.
        
        Returns:
        - tuple: (X_final, y_final) com dados oversampled.
        """
        # Transforma X e y em DataFrames para facilitar
        ...
        # 1) Agrupando combinações (classes compostas)
        ...
        # Definimos n_extra como 30% do total (ajustável)
        n_extra = int(len(X_ros) * 0.3)
        if n_extra <= 0:
            n_extra = 1
        X_synth, y_synth = mlsmote.fit_resample(X_ros, y_ros, n_extra)

        # Converte de volta para numpy; garante que y_final seja numérico
        X_final = X_synth.values
        y_final = np.array(y_synth.values.tolist(), dtype=int)
    ...
    def fit(
        ...
        # Initial oversampling - se multilabel, podemos chamar _oversample_multilabel; senão, o método antigo
        ...
        if protein_ids and var_assoc and (y.ndim == 1 or y.shape[1] == 1):
        ...
        if y.ndim > 1 and y.shape[1] > 1:
            # Para multilabel, contamos a soma de cada coluna
            class_counts = defaultdict(int)
            for col in range(y_smote.shape[1]):
                class_counts[col] = np.sum(y_smote[:, col])
    ...
    def get_class_rankings(self, X: np.ndarray) -> list:
        ...
        - list: List of formatted class rankings for each sample.
        """
        return get_class_rankings_global(self.model, X)
...
# NOVA FUNÇÃO: Plot de barras horizontal para "Prediction Confidence"
import plotly.express as px
import streamlit as st
import pandas as pd
...
def plot_prediction_confidence_bar(df_results: pd.DataFrame) -> None:
    ...
    color_map = {
        ...
        "Medium Confidence (0 - 0.5)": "#E67E22", # Orange
        "High Confidence (0 - 1)": "#3498DB"      # Light Blue
    }
...
if get_faal_domain:
    ...
    if uploaded_fasta_faal is not None and st.sidebar.button("Process FAAL Domain"):
        ...
        os.makedirs(output_faal_dir, exist_ok=True)
        with st.spinner("Processing FAAL domain... (this may take several minutes)"):
            def submit_job(fasta_file, email="user@example.com", title="InterProScanJob"):
                import requests  # Import local para garantir que requests esteja definido
