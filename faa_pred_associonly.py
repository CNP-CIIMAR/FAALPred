...
def main(args: argparse.Namespace) -> None:
    ...
    if os.path.exists(calibrated_model_associated_full_path):
        ...
        logging.info("Top 3 rankings de classes para as primeiras 5 amostras nos dados de associated_variable:")
        for i in range(min(5, len(class_rankings_associated))):
            logging.info(f"Amostra {i+1}: Rankings de classes - {class_rankings_associated[i][:3]}")  # Mostra os top 3 rankings

        # Acessando class_weight do dicionário best_params_associated
        class_weight = best_params_associated.get('class_weight', None)
        # Imprimindo resultados
        logging.info(f"Class weight usado: {class_weight}")
    ...
    if os.path.exists(min_kmers_path):
        ...
        logging.error(f"Arquivo min_kmers não encontrado em {min_kmers_path}. Certifique-se de que o treinamento foi concluído com sucesso.")
        sys.exit(1)
    ...
    if not are_sequences_aligned(predict_alignment_path):
        logging.info("Sequências para previsão não estão alinhadas. Realinhando com MAFFT...")
        ...
        logging.info(f"Arquivo alinhado para previsão encontrado ou sequências já alinhadas: {predict_alignment_path}")
    ...
    logging.info(f"Número de embeddings para previsão gerados: {len(protein_embedding_predict.embeddings)}")
    ...
    if os.path.exists(scaler_associated_path):
        ...
        logging.error("Scalers não encontrados. Certifique-se de que o treinamento foi concluído com sucesso.")
        sys.exit(1)
    ...
    logging.info("Gerando gráfico de dispersão das previsões de novas sequências...")
    ...
    st.header("Gráfico de Dispersão das Previsões")
    ...
    if os.path.exists(scatterplot_path):
        ...
        st.error(f"Gráfico de dispersão não encontrado em {scatterplot_path}")
    ...
    for sequence_id, info in results.items():
        associated_rankings = info.get('associated_ranking', [])
        if not associated_rankings:
            logging.warning(f"Sem dados de ranking associado para a proteína {sequence_id}. Pulando...")
            continue
        ...
        if top_specificity is None:
            logging.warning(f"Sem formatação válida para a proteína {sequence_id}. Pulando...")
            continue
    ...
    headers = ["Nome da Consulta", "Specificidade da Previsão SS", "Confiança da Previsão", "Top 2 Specificidades"]
    df_results = pd.DataFrame(formatted_results, columns=headers)

    # Função para aplicar estilos personalizados
    def highlight_table(df: pd.DataFrame) -> pd.io.formats.style.Styler:
        """
        Aplica estilos personalizados à tabela para exibição no Streamlit.
        
        Parâmetros:
        - df (pd.DataFrame): DataFrame a ser estilizado.
        
        Retorna:
        - pd.io.formats.style.Styler: DataFrame estilizado.
        """
        return df.style.set_table_styles([
            {
                'selector': 'th',
                'props': [
                    ('background-color', '#1E3A8A'),  # Azul escuro para cabeçalhos
                    ('color', 'white'),
            ...
            },
            {
                'selector': 'td',
                'props': [
                    ('background-color', '#0B3C5D'),  # Azul marinho para linhas ímpares
                    ('color', 'white'),
                    ('border', '1px solid white'),
                    ('text-align', 'center'),
                    ('font-family', 'Arial'),
            ...
            },
            {
                ...
                'props': [
                    ('background-color', '#0D4F8B')  # Azul mais escuro ao passar o mouse
                ]
            },
        ])
    ...
    # Injetar CSS para botões de download e ajustar estilos adicionais
    st.markdown(
        """
        <style>
...
st.markdown(
    ...
    <div style="text-align: center; font-family: 'Arial', sans-serif; padding: 30px; background: linear-gradient(to bottom, #f9f9f9, #ffffff); border-radius: 15px; border: 2px solid #dddddd; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); position: relative;">
        <p style="color: black; font-size: 1.5em; font-weight: bold; margin: 0;">
            FAALPred: Predizendo Specificidades de Cadeias de Ácidos Graxos em Fatty Acyl-AMP Ligases (FAALs) Usando Abordagens Integradas de Redes Neurais, Bioinformática e Aprendizado de Máquina
        </p>
        <p style="color: #2c3e50; font-size: 1.2em; font-weight: normal; margin-top: 10px;">
            Anne Liong, Leandro de Mattos Pereira, e Pedro Leão
        </p>
        <p style="color: #2c3e50; font-size: 18px; line-height: 1.8;">
            <strong>FAALPred</strong> é uma ferramenta bioinformática abrangente projetada para predizer 
            a especificidade de comprimento de cadeia de substratos de ácidos graxos, variando de C4 a C18.
        </p>
        <h5 style="color: #2c3e50; font-size: 20px; font-weight: bold; margin-top: 25px;">RESUMO</h5>
        <p style="color: #2c3e50; font-size: 18px; line-height: 1.8; text-align: justify;">
            Fatty Acyl-AMP Ligases (FAALs), identificadas por Zhang et al. (2011), ativam ácidos graxos de comprimentos variados para a biossíntese de produtos naturais. 
            Esses substratos permitem a produção de compostos como nocuolin (<em>Nodularia sp.</em>, Martins et al., 2022) 
            e sulfolipid-1 (<em>Mycobacterium tuberculosis</em>, Yan et al., 2023), com aplicações no tratamento de câncer e tuberculose 
            (Kurt et al., 2017; Gilmore et al., 2012). O Dr. Pedro Leão e sua equipe identificaram vários desses produtos naturais em cianobactérias (<a href="https://leaolab.wixsite.com/leaolab" target="_blank" style="color: #3498db; text-decoration: none;">visite aqui</a>), 
            e FAALpred classifica FAALs por sua especificidade de substrato.
        </p>
        <div style="text-align: center; margin-top: 20px;">
            <img src="data:image/png;base64,{image_base64}" alt="Dominio FAAL" style="width: auto; height: 120px; object-fit: contain;">
            <p style="text-align: center; color: #2c3e50; font-size: 14px; margin-top: 5px;">
                <em>Dominio FAAL de Synechococcus sp. PCC7002, link: <a href="https://www.rcsb.org/structure/7R7F" target="_blank" style="color: #3498db; text-decoration: none;">https://www.rcsb.org/structure/7R7F</a></em>
            </p>
        </div>
    </div>
    """,
...
predict_fasta_file = st.sidebar.file_uploader("Enviar Arquivo FASTA de Previsão", type=["fasta", "fa", "fna"])
...
if custom_word2vec:
    ...
    epochs = st.sidebar.number_input(
        "Épocas", min_value=1, max_value=2500, value=2500, step=100
    )
else:
...
if st.sidebar.button("Executar Análise"):
    ...
    if predict_fasta_file is not None:
        ...
        st.error("Por favor, envie um arquivo FASTA para previsão.")
        st.stop()
...
footer_html = """
<div class="support-text">Suporte por:</div>
<div class="footer-container">
    {}
</div>
<div class="footer-text">
    CIIMAR - Pedro Leão @CNP - 2024 - Leandro de Mattos Pereira (desenvolvedor) - Todos os direitos reservados.
</div>
"""
